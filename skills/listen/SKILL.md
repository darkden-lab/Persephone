---
description: Start listening for Discord messages in the active channel
---

Enter a persistent listening loop on the active Discord channel. Messages from Discord are treated as if the user typed them directly in the CLI.
...
2. Send a notification to the Discord channel confirming listening mode is active:
...
3. Tell the user in the CLI that listening mode is active and all interaction will happen via Discord.
...
4. Enter the listening loop:
   ...
   c. If the wait times out (no message in 5 minutes):
      ...
      - Do NOT send a timeout message to Discord — just keep waiting
   d. Go back to step (a)
...
- **Process Discord messages like CLI input**: If the user asks to read a file, write code, search the web, etc., do it and respond via `send_message`.
